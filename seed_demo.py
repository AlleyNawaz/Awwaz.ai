...
# Add backend to path for direct invocation
sys.path.insert(0, str(Path(__file__).resolve().parent / "backend"))

from sqlalchemy import select, delete
from sqlalchemy.ext.asyncio import AsyncSession
from app.core.config import settings
from app.repositories.database import AsyncSessionLocal, init_db, engine
...
async def seed_database(session: AsyncSession = None) -> dict:
    ...
    try:
        ...
        demo_case_ids = ["A1024", "A1025", "A1026", "A1027", "A1028", "A1011"]

        # Delete dependent rows first
        await session.execute(delete(EvidenceModel).where(EvidenceModel.complaint_id.in_(demo_case_ids)))
        await session.execute(delete(CommitmentModel).where(CommitmentModel.complaint_id.in_(demo_case_ids)))
        await session.execute(delete(RecommendationModel).where(RecommendationModel.complaint_id.in_(demo_case_ids)))
        await session.execute(delete(ComplaintEventModel).where(ComplaintEventModel.complaint_id.in_(demo_case_ids)))
        await session.execute(delete(ComplaintModel).where(ComplaintModel.id.in_(demo_case_ids)))
...
if __name__ == "__main__":
    ...
    print(f"✅ Awwaz demo dataset seeded successfully: {result}")
