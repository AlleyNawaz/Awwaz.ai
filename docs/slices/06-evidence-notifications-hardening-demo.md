...
- `integrations/storage/interface.py`: `StorageService` with `put(key, stream, mime) -> StoredObject`, `open(key) -> stream`, `delete(key)` (idempotent), `exists(key)`. There is deliberately no `move`: a storage key is immutable for the life of its evidence row, so the database never has to be reconciled with a half-completed storage operation. `local.py`: `LocalDiskStorage` rooted at `STORAGE_LOCAL_PATH` (default `backend/var/evidence`, git-ignored). S3-compatible implementation is a documented follow-up using the same interface (D6). `factory.py` selects by `STORAGE_BACKEND=local|s3`.
- `domain/evidence/service.py`:
  ...
  - `attach(db, actor, complaint_id, upload)`: owner or OPERATOR/ADMIN; ≤ `EVIDENCE_MAX_PER_CASE` (default 5) attached per case; key `evidence/{uuid}.{ext}`; row `status=ATTACHED`; `EVIDENCE_ADDED` event (non-progress); audit.
  - `stage(db, actor, conversation_id, upload)`: **the chat path (D16)**, for files attached before a case exists. The conversation must belong to the actor. Same validation; ≤ `EVIDENCE_MAX_PER_CASE` staged per conversation; key `evidence/{uuid}.{ext}` (same layout as attached files; the row, not the key, records whether a file is staged); row `status=STAGED`, `complaint_id=NULL`, `conversation_id` set, `expires_at = now + EVIDENCE_STAGING_TTL_HOURS` (default 24). Returns the evidence ID that the composer later sends as `attachment_ids`.
  ...
  - `bind_staged(db, complaint, conversation_id)`: invoked **inside** `ComplaintService.create_complaint`'s transaction when the agent's `create_case` tool succeeds, and inside the `ADD_INFORMATION` path's transaction when the conversation already has an active case. Every `STAGED` row on the conversation is selected `FOR UPDATE` (so the janitor cannot claim it mid-bind; a row the janitor has already moved to `EXPIRED` is simply not selected) and becomes `ATTACHED` to the complaint (`complaint_id` set, `attached_at` set, `expires_at` cleared) with one `EVIDENCE_ADDED` event per file. Binding touches **no storage object**: keys are immutable, so the bind is a pure database update that commits or rolls back atomically with the case insert. If it rolls back, the rows stay `STAGED`, remain readable by the uploader at their unchanged keys, and remain subject to the janitor's TTL; nothing is orphaned. Staging is per conversation rather than per message, so a file attached to the first message survives any number of clarification turns until the case is created. The per-case cap is enforced at bind time; overflow files stay staged and the reply states how many were attached.
  - `get(db, actor, evidence_id)`: `ATTACHED` → same visibility as the case (A11 for files); `STAGED` → the uploader, OPERATOR, and ADMIN only; `EXPIRED` → 404.
- Worker `evidence_janitor.run()` (added to the scheduler tick), two phases so it can never race `bind_staged`:
  1. **Claim** (one transaction): `SELECT … FROM evidence WHERE status = 'STAGED' AND expires_at < now FOR UPDATE SKIP LOCKED`, re-checking `status` under the lock, then set those rows to `status = 'EXPIRED'` and commit. A row currently locked by an in-flight `bind_staged` is skipped and re-examined next tick, by which time it is `ATTACHED` and no longer matches; a row the janitor has claimed is no longer `STAGED`, so a `bind_staged` that starts afterwards never selects it. No storage call happens in this phase.
  2. **Purge** (outside any lock, per row): for each `EXPIRED` row, delete the storage object (idempotent, so a missing object is fine), then delete the row. A crash between the two leaves an `EXPIRED` row that the next tick purges again. If the object delete fails, the row stays `EXPIRED` for the next tick.
  Audited as SERVICE. An `ATTACHED` row can therefore never lose its object to the janitor, and a purged object can never belong to a row that later becomes `ATTACHED`.
...
```text
evidence          id UUID PK, complaint_id FK NULL, conversation_id FK NULL, status (STAGED|ATTACHED|EXPIRED), storage_key TEXT UNIQUE,
                  ...
                  CHECK ((status = 'ATTACHED' AND complaint_id IS NOT NULL)
                      OR (status IN ('STAGED', 'EXPIRED') AND conversation_id IS NOT NULL AND expires_at IS NOT NULL))
...
- Storage keys never change after upload. The evidence row is the only thing that moves between staged and attached, so binding is atomic with case creation and there is no storage/database reconciliation problem.
- The janitor never touches storage for a row it has not first claimed. It moves `STAGED → EXPIRED` under a row lock and commits before any delete, and `bind_staged` locks the rows it binds, so purging and binding can never both act on the same row.
...
- Evidence (chat path, fixture LLM): stage two images on a conversation → send the Roman Urdu message → clarification → send the location → the created case has both files `ATTACHED`, two `EVIDENCE_ADDED` events, storage keys unchanged, `expires_at` cleared. Bind atomicity: a test hook makes the complaint insert fail after `bind_staged` has run → no complaint row, both evidence rows still `STAGED`, both objects still readable at their original keys. Another citizen's staged ID in `attachment_ids` → 400 naming the ID and no agent run. Expired staged ID → 400. Sixth staged file on one conversation → 400. `STAGED` file: uploader and operator can GET, another citizen → 404. Janitor deletes expired staged rows and objects and leaves unexpired ones. Janitor/bind race, forced in both orders with a test barrier: (a) `bind_staged` holds its `FOR UPDATE` lock on an expired-but-`STAGED` row while the janitor's claim runs → the janitor skips the row, the row ends `ATTACHED` with its object present, and the next janitor tick leaves it alone; (b) the janitor claims first → the row is `EXPIRED`, `bind_staged` attaches zero files and the reply says so, and the purge removes object and row. In neither order does an `ATTACHED` row exist without its object. A crash injected between the janitor's claim commit and its purge leaves an `EXPIRED` row that the next tick purges.
